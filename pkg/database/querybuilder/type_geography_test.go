...
type GeographyTest struct {
	...
	Column2 GeographyPoints `row:"column2" type:"exact"`
}
...
func Test_GeographyGet(t *testing.T) {
	...
	err := queryBuilderCommon(GeographyTest{}, passQuery, func(args []interface{}) bool {
		return len(args) == 0
	}, queryBuilderJoin)
	if err != nil {
		t.Error(err)
...
func Test_GeographyGetWithArgs(t *testing.T) {
	passQuery := `SELECT public.test_model.column1,ST_X(public.test_model.column2::geometry),ST_Y(public.test_model.column2::geometry) FROM public.test_model  WHERE ST_DWithin(test_model.column2,ST_MakePoint(1.1,2.2),3000)`
